...
import { CheckCircle2, XCircle, Loader2, Lock, Server, Cpu, ShieldAlert, Cloud, Mic } from "lucide-react";
import { pingOllama, pingWhisper, clearAIConfigCache, transcribe, type AIProvider } from "@/lib/aiClient";
...
export default function Settings() {
  ...
  const [s, setS] = useState({
    ai_provider: "cloud" as AIProvider,
  ...
  useEffect(() => {
    (async () => {
      ...
      if (settings) setS({
        ai_provider: ((settings as any).ai_provider as AIProvider) ?? "cloud",
  ...
  // Records 4 seconds of mic audio and runs it through the active provider.
  async function testTranscription() {
    setTesting(true);
    setTestResult(null);
    try {
      // Save current settings first so the edge function uses the right provider
      const userId = (await supabase.auth.getUser()).data.user!.id;
      ...
      rec.start();
      toast.info("Recording 4 seconds — say something now");
      ...
      const result = await transcribe(blob, { language: profile.language, mimeType: mime });
      const text = result.text?.trim();
      setTestResult(text || "(no speech detected)");
      if (text) toast.success("Transcription succeeded");
      else toast.warning("Transcription returned empty text");
    } catch (e: any) {
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Choose your AI provider, configure your profile, and manage security</p>
      </div>
      ...
      <Card className="border-border/60">
        <CardHeader>
          ...
          <CardDescription>
            Cloud uses managed AI (no setup, no API keys). Local uses your own Ollama + Whisper servers running on your machine.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid sm:grid-cols-2 gap-3">
            <button
              type="button"
              onClick={() => setS({ ...s, ai_provider: "cloud" })}
              className={`text-left p-4 rounded-lg border transition ${s.ai_provider === "cloud" ? "border-primary bg-primary/5 ring-1 ring-primary/30" : "border-border/60 hover:border-border"}`}
            >
              <div className="flex items-center gap-2 font-medium"><Cloud className="h-4 w-4" /> Cloud (recommended)</div>
              <div className="text-xs text-muted-foreground mt-1">Works immediately. Transcription + AI run via Lovable AI Gateway.</div>
            </button>
            <button
              type="button"
              onClick={() => setS({ ...s, ai_provider: "local" })}
              className={`text-left p-4 rounded-lg border transition ${s.ai_provider === "local" ? "border-primary bg-primary/5 ring-1 ring-primary/30" : "border-border/60 hover:border-border"}`}
            >
              <div className="flex items-center gap-2 font-medium"><Cpu className="h-4 w-4" /> Local</div>
              <div className="text-xs text-muted-foreground mt-1">Self-hosted. Requires Ollama + Whisper running with CORS enabled.</div>
            </button>
          </div>

          <div className="p-3 rounded-md bg-muted/40 border border-border/60 space-y-3">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                <div className="text-sm font-medium flex items-center gap-2"><Mic className="h-4 w-4" /> Test transcription end-to-end</div>
                <div className="text-xs text-muted-foreground">Records 4s of mic audio and sends it to the active provider.</div>
              </div>
              <Button variant="outline" size="sm" onClick={testTranscription} disabled={testing}>
                {testing ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Recording…</> : "Run test"}
              </Button>
            </div>
            {testResult && (
              <div className="text-sm font-mono p-3 rounded bg-background border border-border/60 whitespace-pre-wrap">
                {testResult}
              </div>
            )}
          </div>
